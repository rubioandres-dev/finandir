...
export function VistaCompromisos({
  ...
  datos: DatosDeCompromisos
  monedas: Moneda[]
}) {
  const { t } = useTraduccion()
  const { formatearMonto, formatearFecha, formatearMesCorto } = useFormatoRegional()

  const { curva, planes, error } = datos
  const nombreDeCuenta = new Map(datos.nombrePorCuenta)
    ...
    <div className="flex flex-col gap-5">
      ...
      <div className="grid grid-cols-2 gap-3">
        <Card glass className="glow-gold col-span-2 p-5">
          ...
          <div className="mt-2.5 flex flex-col gap-0.5">
            {totalPorMoneda.filter((t) => t.valor > 0).length === 0 ? (
              <span className="text-lg text-subtle">{t('cuotas.sinPendientes')}</span>
            ) : (
              totalPorMoneda
                .filter((t) => t.valor > 0)
                .map((t) => (
        ...
        <Card glass className="p-4">
          ...
          <p className="mt-1.5 text-[11px] text-subtle">{t('cuotas.porMesConVencimientos')}</p>
        </Card>
        ...
        <Card glass className="p-4">
          ...
          <p className="mt-1.5 text-[11px] text-subtle">{t('cuotas.primerMesLibre')}</p>
        </Card>
      </div>
...
export function CompromisosEnCliente({
  monedas,
  hoy,
}: {
  monedas: Moneda[]
